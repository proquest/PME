...
Zotero.Translate.Base.prototype = {
	...
	"getTranslators":function(getAllTranslators, checkSetTranslator) {
		...
		if(Zotero.isBookmarklet){
		  ...
		  script.setAttribute('src', "%UIConfigURL%");
		  document.getElementsByTagName('body')[0].appendChild(script);
		  _me.injectionCode = "";
		}
		else{
		  Zotero.HTTP.doGet("%UIConfigURL%", function (xmlhttp) {
...
Zotero.Translate.Web.prototype._translateTranslatorLoaded = function() {
	try {
		...
		Zotero.debug(this._sandboxManager);
		this._sandboxManager.eval(this.injectionCode,["entry","selection","single"]);
