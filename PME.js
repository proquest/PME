...
PME.genericScrape = function(doc) {
	...
	function getDoiFromElement(node) {
		// getAttribute is only defined for elements, not all node types
	...
	function crawlDomTree(node) {
		...
		return (DOImatch ? (isArrayLike(DOImatch) ? PME.Util.trim(DOImatch[0] || "") : DOImatch) : false);
	}
...
PME.getPageMetaData = function (callback) {
	try {
		...
		var continuation = function() {
			if (! trans) {
				// no translator matches the url/doc
				completed({ noTranslator: true });
			}
			else if (trans == "lb72u4uo-s4in-8rj8-wlyb-i8zcjef7hlvs") {
		...
		// add XPath shim if no built-in support is available
		if (! pageDoc.evaluate)
			PME.Util.xpathHelper(window, pageDoc, continuation);
		else
			continuation();
	}
	catch(e) {
