...
function getAbstract(doc) {
	//abstract, summary
	var abstrSections = PME.Util.xpath(doc,
	...
	for(var i=0, n=abstrSections.length; i<n; i++) {
		paragraph = PME.Util.getNodeText(abstrSections[i]).trim();
...
function addEmbMeta(doc) {
	...
	//Embedded RDF Metadata translator
		...
		for (i in item.creators){
			if(item.creators[i].lastName == item.creators[i].lastName.toUpperCase()) {
				item.creators[i].lastName =
					PME.Util.capitalizeTitle(item.creators[i].lastName, true);
			}
			if(item.creators[i].firstName == item.creators[i].firstName.toUpperCase()) {
				item.creators[i].firstName =
					PME.Util.capitalizeTitle(item.creators[i].firstName, true);
			}
		}
...
function detectWeb(doc, url) {
	...
	if (!highwiretest) {
		...
		highwiretest = PME.Util.xpath(doc,
...
function doWeb(doc, url) {
	if (hasMultiple(doc, url)) {
		...
		for(var i=0, n=searchres.length; i<n; i++) {
			...
			title = PME.Util.xpathText(next_res, titlex);
			link = PME.Util.xpathText(next_res, linkx);
			if(link && title) {
		...
		PME.selectItems(items, function(selectedItems) {
			...
			PME.Util.processDocuments(urls, addEmbMeta);
		});
	} else {
		addEmbMeta(doc);
	}
