...
function applyValue(item, zField, value, rawLine) {
	...
	if(!PME.parentTranslator //cannot use this in connectors, plus we drop notes in most cases anyway
		...
		switch(entry[1]) {
			...
			case 'TY':
				...
				if(!type) {
					...
					PME.debug("Unknown RIS item type: " + entry[2] + ". Defaulting to " + type);
				}
...
function doExport() {
	...
	while(item = PME.nextItem()) {
		...
		PME.write("ER  - " + newLineChar + newLineChar);
	}
}
