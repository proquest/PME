...
function processTag(item, tag, value) {
	...
	if(fieldMap[tag]) {
		...
		if(dateParts.length == 1) {
			...
			item.date = PME.Util.formatDate({year:dateParts[0],
								  month:month,
								  day:dateParts[2],
								  part:dateParts[3]});
		}
		...
		if(dateParts.length != 4 && item.itemType != "patent") {
			...
			if(dateParts.length == 1) {
				...
				item.filingDate = PME.Util.formatDate({year:dateParts[0],
								  month:month,
								  day:dateParts[2],
								  part:dateParts[3]});
			}
			...
			if(dateParts.length == 1) {
				...
				item.accessDate = PME.Util.formatDate({year:dateParts[0],
								  month:month,
								  day:dateParts[2],
								  part:dateParts[3]});
			}
...
function doImport(attachments) {
	...
	var rawLine;
	while((rawLine = PME.read()) !== false) {    // until EOF
...
function doExport() {
	...
	while(item = PME.nextItem()) {
		...
		// date
		if(item.date) {
			var date = PME.Util.strToDate(item.date);
		...
		// filingDate (patents)
		if(item.filingDate) {
			var date = PME.Util.strToDate(item.filingDate);
			var string = date.year+"/";
			if(date.month != undefined) {
				// deal with javascript months
				date.month++;
		...
		if(item.pages) {
			if(item.itemType == "book") {
				...
				var range = PME.Util.getPageRange(item.pages);
				addTag("SP", range[0]);
				addTag("EP", range[1]);
			}
		}
