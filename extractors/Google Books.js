(function () {
	...
	function doStorebook()
	{
		...
		item.abstractNote = $.parseHTML('<div>' + $('div.details-section.description>meta[itemprop="description"]', doc_).attr("content") + '</div>')[0].innerText
	...
	function doMybooks()
	{
		...
		{
			var url = /(id=.*)(?:&|$)/.exec($(this).attr("href"));
			URLs.push(window.location.protocol + '//' + window.location.host + '/store/books/details?' + RegExp.$1);
		});
		PME.Util.processDocuments(URLs, doWeb);
	...
	function doWeb(doc, url)
	{
		...
		doc_ = doc;
		url_ = url;
		if (url_.match(/books\.google\.com\/books/i))
		{
			if ($('div.vertical_module_list_row:has(h3.about_title)', doc_).size() > 0)
				...
				doSliders();
			else if ($("div.bookshelf-holder").size() > 0)
				doBookshelf();
		}
		else if (url_.match(/play\.google\.com\/store\/books\/details/i))
			doStorebook();
		else if (url_.match(/play\.google\.com\/store\/books/i))
			doStorelist();
