...
function detectWeb(doc, url) {
	...
	var persistentLink = PME.Util.xpath(doc, '//a[@class="permalink-link"]');
	...
	translator.setHandler("itemDone", function(obj, item) {
		...
		for(var i=0, n=item.creators.length; i<n; i++) {
			...
			if(fn && fn.toUpperCase() == fn) {
				item.creators[i].firstName = PME.Util.capitalizeTitle(fn, true);
			}

			ln = item.creators[i].lastName;
			if(ln && ln.toUpperCase() == ln) {
				item.creators[i].lastName = PME.Util.capitalizeTitle(ln, true);
			}
		}
			...
			PME.Util.doGet(pdf, function (text) {
					...
					if(realpdf) {
						...
						if(an) {
							...
							PME.debug("Don't have an accession number. PDF might fail.");
						}
						...
						PME.debug("Could not detect embedded pdf.");
						var m = text.match(/<iframe[^>]+>/i) || text.match(/<embed[^>]+>/i);
						if(m) PME.debug(m[0]);
					}
				},
				function () {
					PME.debug("PDF retrieval done.");
					item.complete(); }
			);
		} else {
			PME.debug("Not attempting to retrieve PDF.");
			item.complete();
		}
	});
...
function getResultList(doc, items, itemInfo) {
	var results = PME.Util.xpath(doc, '//li[@class="result-list-li"]');
	...
	for(var i=0, n=results.length; i<n; i++) {
		...
		itemInfo[title[0].href] = {
			...
			fetchPDF: PME.Util.xpath(results[i], './/span[@class="record-formats"]\
...
function doDelivery(doc, itemInfo) {
	...
	if(!itemInfo)	{
		prefs.fetchPDF = !(PME.Util.xpath(doc, '//div[@id="column1"]//ul[1]/li').length	//check for left-side column
			&& !PME.Util.xpath(doc, '//a[contains(@class,"pdf-ft")]').length);	//check if there's a PDF there
	} else {
	...
	var postURL = PME.Util.xpathText(doc, '//form[@id="aspnetForm"]/@action');
	...
	PME.Util.doGet(postURL, function (text) {
