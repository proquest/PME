...
function getFirstResults(node, properties, onlyOneString) {
	for(var i=0; i<properties.length; i++) {
		var result = PME.RDF.getTargets(node, properties[i]);
		if(result) {
			if(onlyOneString) {
...
function handleCreators(newItem, creators, creatorType) {
	...
	if(typeof(creators[0]) != "string") {	// see if creators are in a container
		try {
			var c = PME.RDF.getContainerElements(creators[0]);
		} catch(e) {}
		if(c && c.length) {
			creators = c;
	}
		...
		for(var i in creators) {
			if(typeof(creators[i]) != "object") {
				...
				if (creators[i].indexOf(',') !== -1) {
					newItem.creators.push(PME.Util.cleanAuthor(creators[i], creatorType, true));
				} else {
					newItem.creators.push(PME.Util.cleanAuthor(creators[i], creatorType, false));
				}
			}
		}
	} else {								// also support foaf
		for(var i in creators) {
			var type = PME.RDF.getTargets(creators[i], rdf+"type");
			if(type) {
...
function processCollection(node, collection) {
	...
	for each(var child in children) {
		var type = PME.RDF.getTargets(child, rdf+"type");
		if(type) {
			type = PME.RDF.getResourceURI(type[0]);
		}
		...
		if(type == n.bib+"Collection" || type == n.z+"Collection") {
			...
			if(isPart(child)) {
				PME.debug("Not adding child item <" + PME.RDF.getResourceURI(child) + "> to collection", 2);
				continue;
			}
			
			// all other items are added by ID
			collection.children.push({id:PME.RDF.getResourceURI(child), type:"item"});
		}
	}
	return collection;
}
	...
	if(subjects = getFirstResults(node, [n.dc+"subject", n.dcterms+"subject"])) {
		for each(var subject in subjects) {
			if(typeof(subject) == "string") {	// a regular tag
				...
				// a call number
				var type = PME.RDF.getTargets(subject, rdf+"type");
				if(type) {
					type = PME.RDF.getResourceURI(type[0]);
...
function getNodeByType(nodes, type) {
	...
	for each(var node in nodes) {
		var nodeType = PME.RDF.getTargets(node, rdf+"type");
		if(nodeType) {
			nodeType = PME.RDF.getResourceURI(nodeType[0]);
...
function isPart(node) {
	var arcs = PME.RDF.getArcsIn(node);
	var skip = false;
	for each(var arc in arcs) {
		arc = PME.RDF.getResourceURI(arc);
...
function detectType(newItem, node, ret) {
	...
	if(type) {
		...
		switch(type) {
			...
			case "attachment":
				...
				if(path) {
					newItem.path = PME.RDF.getResourceURI(path[0]);
				}
	...
	if(type && PME.Util.itemTypeExists(type)) {
		t.zotero = type;
	}
	...
	if(type) {
		if(PME.Util.itemTypeExists(type)) {
			t.dc = type;
		} else {
			switch (type) {
				//eprints
...
function importItem(newItem, node) {
	...
	var possibleCreatorTypes = PME.Util.getCreatorsForType(newItem.itemType);
	var creators;
	...
	if(publisher) {
		if(typeof(publisher[0]) == "string") {
			...
			var type = PME.RDF.getTargets(publisher[0], rdf+"type");
			if(type) {
				type = PME.RDF.getResourceURI(type[0]);
	...
	if(identifiers) {
		for(var i in identifiers) {
			if(typeof(identifiers[i]) == "string") {
				...
				// grab URLs
				var type = PME.RDF.getTargets(identifiers[i], rdf+"type");
				if(type && (type = PME.RDF.getResourceURI(type[0])) && type == n.dcterms+"URI") {
	...
	if(!newItem.url) {
		...
		if(url) {
			newItem.url = PME.RDF.getResourceURI(url[0]);
		}
	}
	...
	var referencedBy = PME.RDF.getTargets(node, n.dcterms+"isReferencedBy");
	...
	for each(var relation in relations) {			
		var type = PME.RDF.getTargets(relation, rdf+"type");
		if(PME.RDF.getResourceURI(type[0]) == n.z+"Attachment") {
			var attachment = new PME.Item();
	...
	/** OTHER FIELDS **/
	var arcs = PME.RDF.getArcsOut(node);
	for each(var arc in arcs) {
		var uri = PME.RDF.getResourceURI(arc);
		if(uri.substr(0, n.z.length) == n.z) {
			var property = uri.substr(n.z.length);
			newItem[property] = PME.RDF.getTargets(node, n.z+property)[0];
		}
	}
...
function getNodes(skipCollections) {
	...
	for each(var node in nodes) {
		...
		if(PME.RDF.getSources(node, n.dcterms+"isPartOf") ||
		   PME.RDF.getSources(node, n.bib+"presentedAt") ||
		   PME.RDF.getSources(node, n.link+"link") ||
		   PME.RDF.getSources(node, n.dcterms+"creator")) {
			continue;
		}
		
		// type
		var type = PME.RDF.getTargets(node, rdf+"type");
		if(type) {
			type = PME.RDF.getResourceURI(type[0]);
...
function doImport() {
	...
	for each(var node in nodes) {
		...
		PME.setProgress(i++/nodes.length*100);
	}
	
	/* COLLECTIONS */
	
	for each(var collection in collections) {
		if(!PME.RDF.getArcsIn(collection)) {
			var newCollection = new PME.Collection();
