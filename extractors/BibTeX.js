...
function detectImport() {
	...
	var re = /^\s*@[a-zA-Z]+[\(\{]/;
	while((buffer = PME.read(4096)) && charsRead < maxChars) {
		PME.debug("Scanning " + buffer.length + " characters for BibTeX");
...
function processField(item, field, value) {
	if(PME.Util.trim(value) == '') return null;
	if(fieldMap[field]) {
		...
		for(var i=0; i<names.length; i++) {
			...
			if (pieces.length > 1) {
				...
				creator = PME.Util.cleanAuthor(name, field, false);
			}
		...
		if(monthIndex != -1) {
			value = PME.Util.formatDate({month:monthIndex});
		} else {
			value += " ";
		}
	...
	//If they do we don't know which is better so we might as well just take the second one
		...
		item.notes.push({note:PME.Util.text2html(value)});
	} else if (field == "pdf") {
	...
	if(value.length > 1) {
		...
		for (var mapped in reversemappingTable) { // really really slow!
			...
			while(value.indexOf(mapped) !== -1) {
				PME.debug("Replace " + mapped + " in " + value + " with " + unicode);
				value = value.replace(mapped, unicode);
			}
			mapped = mapped.replace(/[{}]/g, "");
			while(value.indexOf(mapped) !== -1) {
				//PME.debug(value)
				PME.debug("Replace(2) " + mapped + " in " + value + " with " + unicode);
				value = value.replace(mapped, unicode);
			}
		}
...
function beginRecord(type, closeChar) {
	type = PME.Util.trimInternal(type.toLowerCase());
	if(type != "string") {
		var zoteroType = bibtex2zoteroTypeMap[type];
		if (!zoteroType) {
			PME.debug("discarded item from BibTeX; type was "+type);
			return;
		}
		var item = new PME.Item(zoteroType);
	...
	while(dontRead || (read = PME.read(1))) {
		dontRead = false;
		...
		// read whitespace
			...
			if(keyRe.test(read)) {
				...
				while((read = PME.read(1)) && keyRe.test(read)) {
					value += read;
				}
...
function writeField(field, value, isMacro) {
	...
	if (PME.getOption("exportCharset") != "UTF-8") {
...
function tidyAccents(s) {
	...
	if (PME.Util.removeDiacritics !== undefined)
		r = PME.Util.removeDiacritics(r, true);
	else {
...
function buildCiteKey (item,citekeys) {
	...
	while (citeKeyConversionsRe.test(citeKeyFormatRemaining)) {
		if (counter > 100) {
			PME.debug("Pathological BibTeX format: " + citeKeyFormat);
			break;
		}
		...
		if (typeof(f) == "function") {
			...
			PME.debug("Got value " + value + " for %" + m[1]);
...
function doExport() {
	//PME.write("% BibTeX export generated by Zotero "+PME.Util.getVersion());
	...
	while(item = PME.nextItem()) {
		...
		if(item.publicationTitle) {
			...
			} else if(PME.getOption("useJournalAbbreviation")){
