...
var translatorSpec =
{
	...
	"label": "ProQuest",
	"creator": "Avram Lyon",
	"target": "^https?://.*(search|aa1)\\.proquest\\.com.*\\/(docview|pagepdf|results|publicationissue|browseterms|browsetitles|browseresults|myresearch\\/(figtables|documents))",
	"minVersion": "3.0",
	"maxVersion": "",
	"priority": 100,
	"inRepository": true,
...
function initLang(doc, url) {
	var lang = PME.Util.xpathText(doc, '//a[@id="changeLanguageLink"]/text()');
...
function fetchEmbeddedPdf(url, item, callback) {
	PME.Util.processDocuments(url, function(doc) {
		var pdfLink = PME.Util.xpath(doc, '//span[@class="pdfReader_link"]/a');
		var attr = 'href';

		//try to fall back to the URL of the embedded PDF
		if(!pdfLink.length) {
			PME.debug('PDF link not found. Falling back to embedded PDF.');
			pdfLink = PME.Util.xpath(doc, '//embed');
			attr = 'src';
		}

		if(!pdfLink.length) {
			PME.debug('Could not determine PDF url.');
			PME.debug('Will try to use supplied url: ' + url);
		} else {
			url = pdfLink[0][attr];
		}
...
function detectWeb(doc, url) {
	...
	if (url.indexOf("/results/") === -1) {
		...
		//e.g. pdf view
		var abstract_link = PME.Util.xpath(doc, '//a[@class="formats_base_sprite format_abstract"]');
...
function doWeb(doc, url, pdfUrl) {
	...
	//block, where it is set to false or an actual value
		...
		pdfUrl = false;
		var link = PME.Util.xpathText(doc,
		...
		if(url.indexOf('fulltextPDF') != -1) {
			pdfUrl = PME.Util.xpath(doc, '//embed');
			if(pdfUrl.length) {
				pdfUrl = pdfUrl[0].src;
			} else {
				pdfUrl = false;
