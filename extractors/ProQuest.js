...
function doWeb(doc, url) {
	...
	if (detected && detected != "multiple") {
		...
		items = PME.selectItems(items);
		if(!items) return true;
		for (var i in items) {
			articles.push(i);
		}
		PME.Util.processDocuments(articles, scrape, function () {PME.done();});
		PME.wait();
	}
}
...
function scrape (doc) {
		...
		switch (field) {
			...
			case "Authors":
					item.creators = valueAArray.map(
							function(author) {
								return PME.Util.cleanAuthor(author,
									"author",
			...
			case "Advisor":		// TODO Map when exists in PME
					break;
			case "Source type":
			case "Document Type":
					item.itemType = (mapToPME(value)) ? mapToPME(value) : item.itemType; break;
			case "Copyright":
			...
			default: PME.debug("Discarding unknown field '"+field+"' => '" +value+ "'");
		}
	}
	...
	if (realLink) {
		...
		if (pdf) {
			var pdfDoc = PME.Util.retrieveDocument(pdf.href);
...
function mapToPME (type) {
	...
	PME.debug("No mapping for type: "+type);
	return false;
}
