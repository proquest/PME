...
function setGSPCookie(doc, cookie) {
	...
	doc.cookie = cookie;
	PME.debug('Cookie set to: ' + cookie);
...
function determineType(result) {
	var titleHref =  PME.Util.xpathText(result, './/h3[@class="gs_rt"]/a[1]/@href');
	 ...
	 */
	var subTitle = PME.Util.xpathText(result, './div[@class="gs_a"]');
...
function getViableResults(doc) {
 return PME.Util.xpath(doc, '//div[@class="gs_r"]\
			...
			PME.Util.HTTP.doGet(article.bibtexUrl,
				function(text) {
					...
					translator.setHandler('itemDone', function(obj, item) {
						...
						var snapshotUrl = PME.Util.xpathText(article.result,
							'.//h3[@class="gs_rt"]/a/@href');
						var linkTitle = PME.Util.xpathText(article.result,
							'.//h3[@class="gs_rt"]');
						...
						var pdf = PME.Util.xpath(article.result,
				...
				PME.Util.HTTP.doGet(factory.bibtexLink, function(text) {
					...
					if(!text.match(/title={{}}/i)) {
						...
						if (!factory.hasReporter() && factory.attachmentLinks[0]) {
							...
							__result_counter++;
							(function(doc, factory) {
								PME.Util.processDocuments(factory.attachmentLinks[0], 
									function(doc) {
			...
			PME.Util.HTTP.doGet(bibtexUrl, function(text) {
				...
				bibtexTranslator.setHandler("itemDone", function(obj, item) {
					...
					if(item.title.toUpperCase() == item.title) {
						item.title = PME.Util.capitalizeTitle(item.title);
					}
					...
					var patentUrl = PME.Util.xpathText(result,
 ...
 * result block instead of using Google Patents translator
		var patentUrl = PME.Util.xpathText(patents[i].result,
									'.//h3[@class="gs_rt"]/a[1]/@href');
...
function doWeb(doc, url) {
	...
	if (url.indexOf('/scholar_case?') != -1) {
		...
		for(var i=0, n=results.length; i<n; i++) {
			...
			items[bibtexUrl] = PME.Util.xpathText(results[i], './/h3[@class="gs_rt"]');
...
ItemFactory.prototype.repairTitle = function () {
	...
	if (this.v.title.match(/(?:[^a-z]|^)[A-Z]{4,}(?:[^a-z]|$)/)) {
		this.v.title = PME.Util.capitalizeTitle(this.v.title.toLowerCase())
