...
function fixCase(authorName) {
	if(authorName.toUpperCase() == authorName ||
		...
		return PME.Util.capitalizeTitle(authorName, true);
	}
...
function addCreators(item, creatorType, creators) {
	...
	for(var i=0, n=creators.length; i<n; i++) {
		item.creators.push(PME.Util.cleanAuthor(fixCase(creators[i]),
							creatorType, false));
	}
}
...
function scrapeBook(doc, url, pdfUrl) {
	...
	for( var i=0, n=data.length; i<n; i++) {
		...
		switch(match[1].trim().toLowerCase()) {
			...
			case 'published online':
				var date = PME.Util.strToDate(match[2]);
				date.part = null;
				newItem.date = PME.Util.formatDate(date);
				break;
			case 'print isbn':
			case 'online isbn':
				isbn.push(match[2]);
	...
	newItem.rights = PME.Util.xpathText(doc, '//div[@id="titleMeta"]/p[@class="copyright"]');
	newItem.url = url;
	newItem.abstractNote = PME.Util.trimInternal(
			PME.Util.xpathText(doc, '//div[@id="homepageContent"]\
...
function scrapeEM(doc, url, pdfUrl) {
	...
	var date = PME.Util.xpathText(doc, '//meta[@name="citation_date"]/@content');

	//remove duplicate meta tags
	var metas = PME.Util.xpath(doc,
	...
	translator.setHandler('itemDone', function(obj, item) {
		if( itemType == 'bookSection' ) {
			...
			item.rights = PME.Util.xpathText(doc, '//p[@id="copyright"]');

			//this is not great for summary, but will do for now
			item.abstractNote = PME.Util.xpathText(doc, '//div[@id="abstract"]/div[@class="para"]//p', null, "\n");
		} else {
			var keywords = PME.Util.xpathText(doc, '//meta[@name="citation_keywords"]/@content');
			if(keywords) {
				item.tags = keywords.split(', ');
			}
			item.rights = PME.Util.xpathText(doc, '//div[@id="titleMeta"]//p[@class="copyright"]');
			item.abstractNote = PME.Util.xpathText(doc, '//div[@id="abstract"]/div[@class="para"]', null, "\n");
		}
...
function detectWeb(doc, url) {
	if( url.indexOf('/issuetoc') != -1 ||
		...
		if( url.indexOf('/book/') != -1 ) {
			return 'book';
		} else if ( PME.Util.xpath(doc, '//meta[@name="citation_book_title"]').length ) {
			return 'bookSection';
		} else {
